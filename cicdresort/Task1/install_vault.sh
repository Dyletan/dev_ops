...
if [[ ! "$1" == "transit" ]]; then
    echo "Primary Vault is running on port 8100 with auto-unseal configured"
    echo "Primary Vault Token: /root/.vault-token"
    vault login $(cat /root/.vault-token)
    echo "Check Vault at http:/${SERVER_IP}:8100"
    echo ""
fi